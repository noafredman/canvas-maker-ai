...
## Non API Related Info

### HTML Sub-Element Drag System

The canvas includes a comprehensive drag system for HTML sub-elements within components:

#### Features
- **Unified Transform-Based Dragging**: All element types use CSS transforms for consistent behavior
- **Granulated Text Support**: Individual text spans drag their parent container as a unified component
- **Parent Bounds Detection**: Elements respect their immediate positioned parent's boundaries
- **Performance Optimized**: Bounds calculations cached at drag start to prevent lag
- **Text Selection Handling**: Movement threshold prevents conflicts between text selection and dragging

#### Implementation
```javascript
// Global drag function available in HTML components
window.dragElement = function(event, element) {
    // Handles all element types with unified approach
    // - Detects granulated text spans and moves parent container
    // - Parses existing transform values to prevent jumping
    // - Finds immediate positioned parent for bounds constraints
    // - Caches bounds calculation for smooth performance
};
```
...
<div style="cursor: move;" onmousedown="dragElement(event, this)">
    Draggable container
</div>

<!-- Granulated text spans (drag as one unit) -->
<div onmousedown="dragElement(event, this)">
    <span onmousedown="dragElement(event, this)">Word</span>
    <span onmousedown="dragElement(event, this)">by</span>
    <span onmousedown="dragElement(event, this)">word</span>
</div>
```

#### Supported Element Types
- **Absolutely positioned elements**: Uses transform for smooth movement
- **Relatively positioned elements**: Maintains offset from natural position
- **Inline/text elements**: Individual spans move parent container as one unit
- **Nested containers**: Multi-level hierarchies with proper bounds constraints
- **Complex UI**: Cards, buttons, icons with multiple nesting levels

#### Key Behaviors
- Elements stay within their immediate parent container bounds
- Granulated text spans move together as unified components
- Text selection vs drag differentiated by movement threshold (3px)
- Transform values accumulate correctly across multiple drags
- Performance optimized with cached bounds calculations
